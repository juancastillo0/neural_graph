...
class ShapeHandler {
  ...
  Future<void> handleGestureEvent(Gesture gesture) async {
    if (gesture.gestureType == GestureType.onPanEnd) {
      if (draggedShapes != null) {
        _executeCallbacks(draggedShapes, gesture);
        draggedShapes = null;
      }
      return;
    ...
    if (gesture.gestureType == GestureType.onPanStart) {
      draggedShapes = touchedShapes;
    }
    _executeCallbacks(touchedShapes, gesture);
  }

  void _executeCallbacks(List<Shape> touchedShapes, Gesture gesture) {
